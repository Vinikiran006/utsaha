...
//     ...formData,
//   };
async function submitToGoogleSheet(eventId, eventTitle, formData, whatsappLink) {
  const payload = {
    eventId,
    eventTitle,
    whatsappLink: whatsappLink || "", // This matches the "WhatsApp" header we used in AppScript
    timestamp: new Date().toISOString(),
    ...formData,
  };
...
export default function RegistrationForm({ event }) {
  ...
  const paymentAmount = selectedFee || baseAmount;

  const upiLink = `upi://pay?pa=${UPI_ID}&pn=Utsaha Vaibhava&am=${paymentAmount}&cu=INR&tn=${event.title}`;
  ...
  function handleChange(e) {
    ...
    if (name === "ccCategory" && event.feeCategories) {
      const category = event.feeCategories.find((c) => c.value === value);
      if (category) setSelectedFee(category.fee);
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      await submitToGoogleSheet(event.id, event.title, form, event.whatsappLink); 
  setSubmitted(true);
} catch (err) {
  ...
  function renderCustomFields() {
    return event.registrationFields.map((field) => (
      ...
      >
        ...
        {errors[field.name] && (
          <span className="reg-form__error">{errors[field.name]}</span>
        )}
      </div>
    ));
  }
      ...
      <div className="reg-layout">
        ...
        <form className="reg-form" onSubmit={handleSubmit} noValidate>
          ...
          <div className="reg-form__event-info">
            ...
            {event.registrationFee && (
              <span>
                💰 Fee: <strong>₹{paymentAmount}</strong>
              </span>
            )}
          </div>
